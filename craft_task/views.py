...
class craft_multi_celltype_View(APIView):    
    def post(self, request, *args, **kwargs):
        ...
        if inputtype == 'rundemo':
            ...
            user_input_path_feature = local_settings.USER_PATH + usertask + '/input/' + user_path[analysistype][species]['feature']
        elif inputtype == 'upload':
            assert 'CSV' in request.data.keys() and 'feature' in request.data.keys()
            csvfile = request.FILES['CSV']
            _csv_path = default_storage.save(local_settings.USER_PATH + usertask + '/input/' + csvfile.name, ContentFile(csvfile.read()))
            user_input_path_csv = local_settings.USER_PATH + usertask + '/input/' + csvfile.name
            featurefile = request.FILES['feature']
            _feature_path = default_storage.save(local_settings.USER_PATH + usertask + '/input/' + featurefile.name, ContentFile(featurefile.read()))
            user_input_path_feature = local_settings.USER_PATH + usertask + '/input/' + featurefile.name
        # elif inputtype == 'paste':
        #     assert 'CSVfile' in request.data.keys()
        #     user_input_path_csv = local_settings.USER_PATH + usertask + '/input/csvfile.csv'
        #     with open(user_input_path_csv, 'w') as file:
...
class craft_topology_View(APIView):    
    def post(self, request, *args, **kwargs):
        ...
        if inputtype == 'rundemo':
            is_demo_input = True
            shutil.copy(
                local_settings.DEMO_ANALYSIS + demo_path[analysistype][species]['gene_coord'], 
                local_settings.USER_PATH + usertask + '/input/' + user_path[analysistype][species]['gene_coord'])
            user_input_path_csv = local_settings.USER_PATH + usertask + '/input/' + user_path[analysistype][species]['gene_coord']
        elif inputtype == 'upload':
            assert 'gene_coord' in request.data.keys()
            gene_coord_file = request.FILES['gene_coord']
            _path = default_storage.save(local_settings.USER_PATH + usertask + '/input/' + gene_coord_file.name, ContentFile(gene_coord_file.read()))
            user_input_path_csv = local_settings.USER_PATH + usertask + '/input/' + gene_coord_file.name
...
def viewtaskresultlog(request):
    ...
    if task_obj.analysis_type in ['Single Celltype Mode', 'Multi-Celltype Mode']:
        ...
        craft_result_log = task.get_topo_job_result('Failed', task_obj.output_result_path)
        craft_result_keys = craft_result_log.keys()
        if len(craft_result_log) == 0:
            craft_result_keys = ['craft_result_key']
            craft_result_log = {
...
def viewtaskresult(request):
    ...
    if task_obj.analysis_type in ['Single Celltype Mode', 'Multi-Celltype Mode']:
        ...
        craft_result = task.get_topo_job_result('Success', task_obj.output_result_path)
        craft_result_keys = craft_result.keys()
...
class view_vis_topology_graphlist(APIView):
    def get(self, request, *args, **kwargs):
        ...
        KNN_list = [i.split('/')[-1] for i in glob.glob(task_obj.output_result_path + 'KNN/*')]
        for i in KNN_list: 
            if '.pkl' in i:
                graph_types.append('KNN (k = ' + i[:-4] + ')')
        KNN_SNN_list = [i.split('/')[-1] for i in glob.glob(task_obj.output_result_path + 'KNN_SNN/*')]
        for i in KNN_SNN_list: 
            if '.pkl' in i:
                graph_types.append('KNN-SNN (k = ' + i[:-4] + ')')
        graph_types.append('MST')
        RNN_list = [i.split('/')[-1] for i in glob.glob(task_obj.output_result_path + 'RNN/*')]
        for i in RNN_list: 
            if '.pkl' in i:
                graph_types.append('RNN (r = ' + i[:-4] + ')')
        RNN_SNN_list = [i.split('/')[-1] for i in glob.glob(task_obj.output_result_path + 'RNN_SNN/*')]
        for i in RNN_SNN_list: 
            if '.pkl' in i:
                graph_types.append('RNN-SNN (r = ' + i[:-4].split('_')[0] + ', k = ' + i[:-4].split('_')[1] + ')')
...
def process_path(x: string):
    ...
    if 'KNN' in x or 'KNN-SNN' in x or 'RNN' in x:
        if 'KNN-SNN' in x: return 'KNN_SNN/' + x.split('=')[1].strip()[:-1]
        if 'KNN' in x: return 'KNN/' + x.split('=')[1].strip()[:-1]
        if 'RNN' in x: return 'RNN/' + x.split('=')[1].strip()[:-1]
...
class view_vis_topology(APIView):    
    def get(self, request, *args, **kwargs):
        ...
        graph_selection_str = querydict['graph_selection_str'] # 1NN, KNN (k = 5), ...
        task_obj = craft_task.objects.get(id = taskid)
        ...
        mst_parentchild_relation = generate_parent_child_relation(task_obj.output_result_path + 'MST/')
